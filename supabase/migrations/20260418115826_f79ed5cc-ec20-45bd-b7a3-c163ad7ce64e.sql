-- Make platform filtering in dashboard RPCs case-insensitive and tolerant
-- of label variants (e.g. "google" matches "Google Ads", "meta" matches
-- "Facebook"/"Instagram"). The frontend now sends canonical lowercase keys
-- like 'meta', 'google', 'tiktok' for route-scoped queries, but BigQuery
-- stores raw vendor labels — so we normalize both sides inside the RPC.
...
RETURNS TABLE(
  date date, platform text, publisher_platform text, campaign_id text,
  campaign_name text, campaign_type text, campaign_objective text,
  audience_type text, impressions numeric, clicks numeric, cost numeric,
  conversions numeric, conversions_all numeric, conversions_lower_funnel numeric,
  conversions_upper_funnel numeric, conversion_value numeric, reach numeric,
  landing_page_views numeric, outbound_clicks numeric, video_views numeric,
  video_p100 numeric
)
LANGUAGE plpgsql
    ...
    and (
      ...
      or EXISTS (
        ...
        where
          ...
          -- or vendor label contains the canonical key (e.g. "Google Ads" ~ "google")
          or lower(coalesce(d.platform, '')) like '%' || pk || '%'
          -- or Meta family (facebook/instagram raw labels)
          or (pk = 'meta' and (
