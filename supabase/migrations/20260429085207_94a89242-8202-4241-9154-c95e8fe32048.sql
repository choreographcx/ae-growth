...
declare
  ...
  _excluded_names text[] := ARRAY[
    'page_engagement','post_engagement','video_view','photo_view','like','comment',
    'onsite_conversion.post_net_comment','viewcontent','view_content','page_view',
    'onsite_conversion.total_messaging_connection',
    'onsite_conversion.messaging_conversation_started_7d',
    'onsite_conversion.messaging_conversation_replied_7d',
    'onsite_conversion.messaging_first_reply',
...
begin
  ...
  select
    ...
    case when lower(coalesce(d.conversion_name,'')) = ANY(_excluded_names) then 'excluded'
         else coalesce(d.conversion_funnel_group, '(unspecified)') end as conversion_funnel_group,
    coalesce(sum(d.conversions_all), 0)::numeric as conversions_all
  from public.dashboard_conversions d
  where d.date between p_start and p_end
    and d.conversion_name is not null
    and length(trim(d.conversion_name)) > 0
    and coalesce(d.conversions_all, 0) > 0
    and (_platform_keys is null or EXISTS (
      select 1 from unnest(_platform_keys) as pk where
        lower(coalesce(d.platform,'')) = pk
        or lower(coalesce(d.platform,'')) like '%' || pk || '%'
  ...
  group by d.platform, d.conversion_name,
           case when lower(coalesce(d.conversion_name,'')) = ANY(_excluded_names) then 'excluded'
                else coalesce(d.conversion_funnel_group, '(unspecified)') end
  order by d.platform, 4 desc;
end;
$function$;