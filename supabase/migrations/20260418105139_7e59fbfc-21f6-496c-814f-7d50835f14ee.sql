-- Optimize suppression filtering to avoid statement timeouts.
-- The previous version wrapped the BigQuery FDW scan in a CTE with a per-row
-- EXISTS subquery, which prevented predicate pushdown. We instead materialize
-- the suppression list into a TEMP-style array via a CTE and use a LEFT JOIN
-- with an aggregated CASE, keeping the base scan filterable.
...
RETURNS TABLE(
  date date, platform text, publisher_platform text, campaign_id text, campaign_name text,
  campaign_type text, campaign_objective text, audience_type text,
  impressions numeric, clicks numeric, cost numeric,
  conversions numeric, conversions_all numeric, conversions_lower_funnel numeric, conversions_upper_funnel numeric,
  conversion_value numeric, reach numeric, landing_page_views numeric, outbound_clicks numeric,
...
begin
  ...
  -- Materialize suppression list into a flat lowercase "platform|name" array.
...
-- Same optimization for the breakdown function: precompute the array once.
