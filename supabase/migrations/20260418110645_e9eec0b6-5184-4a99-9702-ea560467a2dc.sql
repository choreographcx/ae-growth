-- Raise statement_timeout inside the long-running BigQuery-backed RPCs.
-- These are SECURITY DEFINER and use SET LOCAL so the change only applies
-- for the duration of the function call, not the whole session.
...
begin
  -- Allow up to 3 minutes for this BigQuery-backed query.
