...
begin
  ...
  -- Skip auth.uid() check so this can be run from the Supabase SQL Editor
  -- (which runs as the postgres role, not a signed-in app user).
  -- Access is still limited via REVOKE/GRANT below.
