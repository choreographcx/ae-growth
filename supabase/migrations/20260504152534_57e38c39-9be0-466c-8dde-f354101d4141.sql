...
begin
  ...
  select
    ...
    coalesce(sum(coalesce(d.lower_funnel_conversions, d.conversions_lower_funnel)), 0)::numeric as conversions,
    coalesce(sum(coalesce(d.all_conversions, d.conversions_all)), 0)::numeric as conversions_all,
    coalesce(sum(coalesce(d.lower_funnel_conversions, d.conversions_lower_funnel)), 0)::numeric as conversions_lower_funnel,
    coalesce(sum(coalesce(d.upper_funnel_conversions, d.conversions_upper_funnel)), 0)::numeric as conversions_upper_funnel,
    coalesce(sum(d.conversion_value), 0)::numeric,
    coalesce(sum(d.reach), 0)::numeric,
    0::numeric,
    coalesce(sum(coalesce(d.lpv, d.landing_page_views)), 0)::numeric as landing_page_views,
