-- Secure wrapper over BigQuery foreign table for dashboard metrics
...
revoke all on function public.get_dashboard_daily(date, date) from public;
revoke all on function public.get_dashboard_daily(date, date) from anon;
grant execute on function public.get_dashboard_daily(date, date) to authenticated;