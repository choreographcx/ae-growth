...
export function CurrencySymbol({ currency, size, className = '' }: { currency: string; size?: number; className?: string }) {
  ...
  if (currency === 'AED') return <span className={className} style={{ fontFamily: 'UAEDirham, sans-serif', fontSize: `calc(${emSize} * 0.75)`, fontWeight: 'bold', verticalAlign: 'baseline' }}>&#xEA;</span>;
